...
load('data/simulated_correspondences_US.mat');
...
for itr_outlier=2:size(store_R,2)
    ...
    for nr_run=1 %1:size(store_R,1)
        ...
        scale_gt=cell2mat(store_s(nr_run,itr_outlier));

        % Set mininum inlier ratio
        min_inlier_ratio=0.01;  % at most 99% outliers

        % Warm-up
        [~,  ~, ~, ~] = SUCOFT_US(pts_3d,pts_3d_,noise,min_inlier_ratio);

        % Running SUCOM for known-scale PCR
        tic;
        [scale_opt, R_opt, t_opt, ~] = SUCOFT_US(pts_3d,pts_3d_,noise,min_inlier_ratio);
        ...
        s_error(nr_run,itr_outlier)=norm(scale_opt - scale_gt);
        ...
        disp(['Scale error of SUCOFT: ', num2str(s_error(nr_run,itr_outlier))]);
